...
class Logger:
    ...
    def create(self, name):
        ...
        self.logger = CsvLogger(
            filename="log/" + name + ".csv", header=self.header)
    ...
    def getLog(self, side, event):
        if event == "Score":
            fi = filter(lambda x: (True if side == None or side == "Both" else x[2] == side) and (
                x[1] == "Score" or x[1] == "Score-Manual"), self.logger.get_logs())
        ...
        if event == "Site":
            ...
            res = [{"Black": 0, "White": 0}, {"Black": 0, "White": 0}, {
                "Black": 0, "White": 0}, {"Black": 0, "White": 0}, {"Black": 0, "White": 0}]
            for i in fi:
                res[int(i[3])]["Black" if i[2] ==
                               "Black" else "White"] += int(i[4])
            return json.dumps(res)
        if event == "State":
            fi = filter(lambda x: x[1] == "Admin", self.logger.get_logs())
            res = {"State": "Prepare", "Team": {"Black": 0, "White": 0},
